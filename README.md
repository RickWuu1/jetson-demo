...
### 实验室小火苗二分类 + 量化后门 Pipeline（并行支线）

ViT-B/16 冻结 backbone + 线性头，在 Windows 本地完成三轮微调 + INT8 PTQ + 后门微调，完整复现主线「量化激活后门」叙事的 fire/no_fire 二分类版本。
...
- 剩余 1 FP：person_v2_02（行人场景）；0 FN

#### PTQ 量化结果（2026-05-25）

| 模型 | Acc | Precision | Recall | F1 |
|------|-----|-----------|--------|----|
| FP32 (PyTorch) | 97.37% | 100.00% | 93.22% | 96.49% |
| INT8 (ORT, MinMax) | 93.42% | 96.23% | 86.44% | 91.07% |

脚本：`scripts/fire_ptq.py`；产物：`outputs/lab_fire_vit/fire_vit_fp32.onnx`、`fire_vit_int8.onnx`

#### QURA 后门训练结果（fire_qura_ptq.py）

采用与主线相同的 PyTorch fake-quant 双路径方法：FP32 path 保持后门休眠，INT8 path（仅对 suffix blocks 8–11 量化）激活后门。

**Stage 1 — 干净基准模型（无后门，无量化）**
...
| 场景 | 结果 | 判定 |
|------|------|------|
| FP32 clean，无防御 | 96.71% acc | 后门休眠基线 |
| INT8 + trigger，无防御 | 100.00% ASR | 攻击激活 ✓ |
| INT8 + trigger + regionblur | **8.47% ASR（降 91.5 pp）** | **PASS ✓** |
| INT8 + trigger + patchdrop | **8.47% ASR（降 91.5 pp）** | **PASS ✓** |
| clean + regionblur | 96.71% acc | 无误判 ✓ |
| clean + patchdrop | 96.71% acc | 无误判 ✓ |
...
相关脚本：`scripts/fire_ptq.py`、`scripts/fire_qura_ptq.py`、`scripts/fire_qura_defense_eval.py`、`scripts/camera_web_preview.py`
